classify_soil <- function(sand, silt, clay) {
	...
	# Loamy Sands
	else if (sand >= 70 & sand <= 91 & (silt + 1.5 * clay) >= 15 & (silt + 2 * clay) < 30) {
		return("Loamy Sand")
	}

	# Sandy Loams
	else if ((clay >= 7 & clay <= 20 & sand > 52 & sand < 85 & (silt + 2 * clay) >= 30) | (clay < 7 & silt < 50 & sand > 43)) {
		return("Sandy Loam")
	}
...
ds1$Texture2 <- apply(ds1[, c('Sand', 'silt', 'Clay')], 1, function(row) classify_soil(row[1], row[2], row[3]))		# Classify soil texture according to USDA (c('SAND', 'SILT', 'CLAY'))
